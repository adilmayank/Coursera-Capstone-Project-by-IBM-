...
spacex_df = pd.read_csv("SpaceEX.csv")
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(filtered_df, values='class',
                     names='Launch Site',
                     title='Successful vs Unsuccessful launches of all sites')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        result = filtered_df['Mission Outcome'].value_counts()
        names=result.index
        values=result.values
        fig = px.pie(filtered_df, values=values,
                     names=names,
                     title='Successful vs Unsuccessful launches by site')
        return fig
    ...
    if entered_site == 'ALL':
        if payload_range is not None:
            filtered_df = filtered_df[filtered_df['Payload Mass (kg)'] \
                .between(payload_range[0], payload_range[1])]
        else:
            pass
